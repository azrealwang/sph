function [code, model] = sph(data, opts, maxIters, model)
% If no model is provided, learn the model first
if nargin == 3 
    model = trainCMRSH(data, opts, maxIters);
end
% Do hash encoding
K = opts.K;
L = opts.L;
...
code.Hy = Hy;
%%=========================================================================
function model = trainCMRSH(data, opts, maxIters)
X = data.X;
Y = data.Y;
S = data.S;
